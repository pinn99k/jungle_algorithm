...
def manage_grades(students):
    ...
    # TODO: 최고 점수 학생 찾기
    top_name = max(students,key=students.get) # get()과 get의 차이
    max_score = students.get(top_name) #students[top_name] 도 가능

    #print(f"평균 점수: {avg}")
    #print(f"최고 점수: {name} ({temp}점)")
    
    return avg, top_name, max_score
